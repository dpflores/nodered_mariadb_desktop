        ...
        oneditprepare: function() {
            ...
            var clickableShapesList = $("#node-input-clickable-container").css('min-height','150px').css('min-width','450px').editableList({
                ...
                header: $("<div>").css({"margin-left":"28px","margin-right":"10x","padding-right":"0px"}).append($.parseHTML(
                   "<div style='width:17%; display: inline-grid'><b>Selector</b></div>" +
                   "<div style='width:25%; display: inline-grid'><b>Action</b></div>" +
                ...
                addItem: function(container, i, clickableShape) {
                    ...
                    var selectorField = $('<input/>',{class:"node-input-clickable-selector",type:"text",placeholder:"Element Selector"}).css({"width":"17%","margin-left":"5px","margin-right":"5px"}).appendTo(row);
                    ...
                    var actionField = $('<select/>',{class:"node-input-clickable-action",type:"text",placeholder:"click"}).css({"width":"25%","margin-left":"5px","margin-right":"5px"}).appendTo(row);
                    ...
                    var payloadField = $('<input/>',{class:"node-input-clickable-payload",type:"text",placeholder:"payload"}).css({"width":"30%","margin-left":"5px","margin-right":"5px"}).appendTo(row);
            ...
            var javascriptHandlersList = $("#node-input-javascript-container").css('min-height','150px').css('min-width','450px').editableList({
                header: $("<div>").append($.parseHTML(
                   ...
                   "<div style='width:60%; margin-left:5px; display: inline-grid'><b>Javascript code</b></div>")),
                addItem: function(container, i, javascriptHandler) {
                    // Add a new row to the editableList
                    var row = $('<div/>').appendTo(container);
                    
                    // Column 1 : Add an input field (type string) to the new row, that represents the selector for this event to operate on.
                    var selectorField = $('<input/>',{class:"node-input-javascript-selector",type:"text",placeholder:"Element Selector"}).css({"width":"17%","margin-left":"5px","margin-right":"5px"}).appendTo(row);
                    ...
                    // Column 2 : Add an input field (type option) to the new row, that represents the action type 
                    var actionField = $('<select/>',{class:"node-input-javascript-action",type:"text",placeholder:"click"}).css({"width":"20%","margin-left":"5px","margin-right":"5px"}).appendTo(row);
                    for(var val in actionOptions) {
                        $('<option />', {value: val, text: actionOptions[val]}).appendTo(actionField);
                    }
                    // A js event handler can also be triggered when an input message arrives
                    $('<option />', {value: "msg", text: "Input msg"}).appendTo(actionField);
            ...
            var bindingList = $("#node-input-binding-container").css('min-height','150px').css('min-width','450px').editableList({
                header: $("<div>").css({"margin-left":"6px"}).append($.parseHTML(
                    ...
                    "<div style='width:24%; margin-left:1px; display: inline-grid'><b>Selector</b></div>" +  
                    "<div style='width:40%; margin-left:1px; display: inline-grid'><b>Binding Destination</b></div>" 
                )),
                addItem: function(container, i, binding) {
                    ...
                    $('<option />', {value: "style", text: "Style"}).appendTo(bindingTypeField);
    ...
    <div id="node-svg-tabs-content" style="min-height: 150px">
        ...
        <div id="node-svg-tab-settings" class="node-svg-tab-content">
            ...
            <div class="form-row">
                ...
                <label for="node-input-showBrowserEvents" style="width:70%;"> Show browser events on the server</label>
            </div>
            <div class="form-row">
                <input type="checkbox" id="node-input-enableJsDebugging" style="display: inline-block; width: auto; vertical-align: top;">
                <label for="node-input-enableJsDebugging" style="width:70%;"> Enable JS event debugging</label>
            </div>
            <div class="form-row">
                <input type="checkbox" id="node-input-sendMsgWhenLoaded" style="display: inline-block; width: auto; vertical-align: top;">
                <label for="node-input-sendMsgWhenLoaded" style="width:70%;"> Send output msg when the client is (re)loaded</label>
            </div>
            <div class="form-row">
                <input type="checkbox" id="node-input-noClickWhenDblClick" style="display: inline-block; width: auto; vertical-align: top;">
                <label for="node-input-noClickWhenDblClick" style="width:70%;"> Don't send click events in case of a double click event</label>
            </div>
            ...
            <div class="form-row">
                <label for="node-input-panning"><i class="fa fa-arrows"></i> Panning</label>
                <select id="node-input-panning">
                    <option value="disabled">Disabled</option> 
                    <option value="both">Pan both X and Y</option>
                    <option value="x">Pan only X</option>
                    <option value="y">Pan only Y</option>
                </select>
            </div>
            <div class="form-row">
                <label for="node-input-zooming"><i class="fa fa-search-plus"></i> Zooming</label>
            ...
            <div class="form-row">
                ...
                <label for="node-input-panOnlyWhenZoomed" style="width:70%;"> Pan only when zoomed</label>
            </div>      
            <div class="form-row">
                <label>&nbsp;</label>
            ...
            <div class="form-row">
                ...
                <span style="vertical-align: super;"> Enable double click zooming by</span>
...
<script type="text/html" data-help-name="ui_svg_graphics">
    ...
    <p><b>See our readme page on <a target="_blank" href="https://github.com/bartbutenaers/node-red-contrib-ui-svg">Github</a> for more information!</b></p>
    ...
    <dl class="message-properties">
        ...
        <div style="padding-left: 15px">
            ...
            <p><i><strong>CAUTION</strong>: it is not possible to use CSS selectors here, but only a single SVG shape (via the Target Element Id field)!</i> It is only possible (in the input messages) to select multiple animations at once via CSS selectors (as explained below).</p>
        </div>        
        <h4>Event</h4>
        <div style="padding-left: 15px">
        ...
        <div style="padding-left: 15px">
            <i>Note that (in the "Selector" field) it is possible to specify the id of a single SVG shape, or multiple SVG shapes via a <a target="_blank" href="https://www.w3schools.com/cssref/css_selectors.asp">CSS selector</a>!</i>  In the latter case, all the SVG shapes - that match the CSS selector - will respond to the specified event.
        </div>
        <h4>Binding</h4>
        <div style="padding-left: 15px">
            ...
            <ul>
                ...
                <li><b>Selector</b> - What elements this binding should work on (e.g. <code>.indicators</code> would affect all elements in the SVG with class="indicators").  Any valid <a target="_blank" href="https://www.w3schools.com/cssref/css_selectors.asp">CSS selector</a> can be used to select one or multiple SVG shapes.</li>
                <li><b>Destination</b> - 
                    <ul>
                        ...
                        <li>If <code>Style</code> is selected, the style attribute specified in the next input will be updated with the value of the binding source.</li>
                    </ul>    
                </ul>
            </ul>
        ...
        <h4>JS</h4>
        <div style="padding-left: 15px">
            A customisable list of Javascript event handlers. Each event targets an element where you can specify what action (e.g. click, mouseover, focus, etc) and a Javascript code snippet to execute upon the specified action.
        </div>
        <div style="padding-left: 15px">
            <i>Note that (in the "Selector" field) it is possible to specify the id of a single SVG shape, or multiple SVG shapes via a <a target="_blank" href="https://www.w3schools.com/cssref/css_selectors.asp">CSS selector</a>!</i>  In the latter case, all the SVG shapes - that match the CSS selector - will respond to the specified event.
        </div>
        <h4>CSS</h4>
        <div style="padding-left: 15px">
            CSS styles whose scope will be limited to this SVG source only!.
        </div>
    </dl>
    <h3>Output</h3>
    <dl class="message-properties">
    ...
    <dl class="message-properties">
        ...
        <dd>It is possible to dynamically trigger all kind of actions on the SVG drawing, via a <code>msg</code></dd>       
        <dd>General guidelines about input messages: 
            <ul>
                <li>A <code>command</code> specifies which operation to perform.</li>
                <li>An <code>elementId</code> or a <code>selector</code> specifies on which element(s) the operation needs to be performed.  An <code>elementId</code> is used to specify a single element id.  While a <code>selector</code> is a <a target="_blank" href="https://www.w3schools.com/cssref/css_selectors.asp">CSS selector</a>, which can be used to select multiple SVG elements at once.  Note that a selector can also be used to select a single element id via a hashtag ( <i>"#some_element_id"</i> ).</li>
                <li>A <code>payload</code> can contain a single command object, or an array of multiple command objects.</li>
                <li>It is possible to specify the command and the selector in the message <code>topic</code>, and only the new value in the payload:
 ...
 return msg;</pre>
        ...
        <dd>
            <p><b>Trigger an animation (via a "start" or "stop" action):</b><br>
                <pre style="white-space: pre">"payload": [
 {
    ...
    "action": "start"
                ...
                <p><i>Notes:</i><br>
                    <ul>
                        <li>The <code>action</code> can be <i>"start"</i> or <i>"stop"</i>.</li>
                        <li>This will only work for animations that have been specified in the "Animations" tabsheet, with trigger <i>"Input message"</i>!</li>
                    </ul>
                </p>
            </p>
            
            <p><b>Update or set an attribute value:</b><br>
                <pre style="white-space: pre">"payload": [
 {
 ...
 },
 {
    ...
    "attributeValue": "inline"
                ...
                <p><i>Notes:</i><br>
                    <ul>
                        <li>Use <code>attributeName</code> in combination with <code>attributeValue</code> to update the (string or number) value of the specified attribute.</li>
                        <li>An attribute can be removed by sending an empty string for the attributeValue.</li>
                        <li><code>set_attribute</code> can be used in place of <code>update_attribute</code>, when a new attribute should be create if it does not exist yet.</li> 
                        <li><code>update_attribute</code> will only change an existing attribute, but do nothing when the attribute doesn't exist yet.</li>
                    </ul>
                </p>
            </p>
            
            <p><b>Set a style attribute value:</b><br>
                <pre style="white-space: pre">payload": [
 {
 ...
 },
 {
    ...
    "style": {"display": "inline", "fill": "blue", "transform": "rotate(5deg)"} 
                ...
                <p><i>Notes:</i><br>
                    <ul>
                        <li>Use <code>attributeName</code> in combination with <code>attributeValue</code> to update the (string or number) value of the specified style attribute.</li>
                        <li>Use <code>style</code> to update the entire style object, i.e. update all the style attribute values with a single command.</li>
                        <li>A style attribute can be removed by sending an empty string for the <code>attributeValue</code> or <code>style</code> value.</li>
                        <li>The command <code>set_style</code> can be used in place of <code>update_style</code>, which perform exactly the same functionality.</li>
                    </ul>
                </p>                
            </p>

            <p><b>Set or get text content</b><br>
                <pre style="white-space: pre">"payload": [
 {
    "command": "update_text",
    "selector": "#myRect > .faultMessage",
    "text": "Hello from a command message"
                ...
                <p><i>Notes:</i><br>
                    <ul>
                        <li>Instead of specifying a <code>text</code> field, it is also possible to use the aliases <code>html</code> or <code>textContent</code>.</li>
                        <li>Use the command <i>"get_text"</i>field to get the current text content.</li>
                        <li>The <code>text</code> can contain HTML or SVG markup.</li>
                    </ul>
                </p> 
            </p>
            
            <p><b>Add or remove events</b><br>
                <pre style="white-space: pre">"payload": [
 {
    "command": "add_event",
    "event": "click",
    ...
    "topic": "CIRCLE_CLICKED"
                ...
                <p><i>Notes:</i><br>
                    <ul>
                        <li>An event handler will be added to the selected elements, to handle the specified <code>event</code>.</li>
                        <li>As soon as the event occurs, an output message will be sent containing the specified <code>payload</code> and <code>topic</code>.</li>
                        <li>When command <i>"remove_event"</i> is used, only the <code>event</code> and <code>selector</code> are required.</li>
                    </ul>
                </p> 
            </p>
            
            <p><b>Add or remove Javascript events</b><br>
                <pre style="white-space: pre">"payload": [
 {
    "command": "add_js_event",
    "event": "click",
    "selector": "#circle_2", 
    "script": "alert('Click event handled on the client ...')"
 }
]</pre>
                <p><i>Notes:</i><br>
                    <ul>
                        <li>An event handler will be added to the selected elements, to handle the specified <code>event</code>.</li>
                        <li>As soon as the event occurs, the specified javascript <code>script</code> will be executed on the client (i.e. inside the dashboard).</li>
                        <li>When command <i>"remove_js_event"</i> is used, only the <code>event</code> and <code>selector</code> are required.</li>
                    </ul>
                </p> 
            </p>
            
            <p><b>Add or remove elements</b><br>
                <pre style="white-space: pre">"payload": [
 {
   "command": "add_element", 
   "elementType": "circle",
   ...
   "textContent": "my content"
                ...
                <p><i>Notes:</i><br>
                    <ul>
                        <li>When a <code>parentElementId</code> is specified, the new element will be added as a child element of that parent element.  When not specified, the new element will be added directly below the root SVG element.</li>
                        <li>When a <code>parentSelector</code> is specified, a new element will be added as a child element of EVERY parent element.  In this case it is not allowed to specify an <code>elementId</code>, because multiple elements are added (which are not allowed to have the same element id).</li>
                        <li>When there is already an existing element (at the same parent) with the same <code>elementId</code>, then the existing element will be replaced by the new one.</li>
                        <li>When command <i>"remove_element"</i> is used, only the <code>elementId</code> is required.  Or a <code>selector</code> if multiple elements need to be removed with a single command.</li>
                    </ul>
                </p> 
            </p>
            
            <p><b>Zoom in or out</b><br>
                <pre style="white-space: pre">"payload": [
 {
    "command": "zoom_in"
 }
                ...
                <pre style="white-space: pre">"payload": [
 {
   ...
   "y": 400
                ...
                <p><i>Notes:</i><br>
                    <ul>
                        <li>The <i>"zoom_in"</i> <i>"zoom_out"</i> commands use fixed zoom percentages.</li>
                        <li>The <i>"zoom_by_percentage"</i> command can be used to specify a custom <code>percentage</code>.</li>
                        <li>The <i>"zoom_by_percentage"</i> command can have optional <code>x</code> and <code>y</code> coordinates, to zoom in an a specific location.</li>
                    </ul>
                </p> 
            </p>
                ...
                <pre style="white-space: pre">"payload": [
 {
   ...
   "y": 400
                ...
                <p><i>Notes:</i><br>
                    <ul>
                        <li>The <i>"pan_to_point"</i> command is being used for absolute panning (to a specified location).</li>
                        <li>The <i>"pan_to_direction"</i> command is being used for relative panning (in a specified direction).</li>
                    </ul>
                </p> 
            </p>
        </dd>
